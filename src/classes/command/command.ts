...
export abstract class classCommand {
	 ...
	 * the current version of Uniffo, and a reference to the documentation.
	 ...
	 * Uniffo, and a header indicating "Documentation:".
	 */
	public getIntroForDocumentation() {
		...
		intro += '					 \n';
		intro += `Uniffo version ${getCurrentCliVersion()}\n\n`;
		intro += `Documentation:\n`;
	 ...
	 * Uniffo project and logs an error message if not.
	...
	public async onlyInsideProject() {
		...
		if (!_pwd) {
			throw `Operation cancelled! The command "${this.getPhrase()}" can only be executed within the scope of the Uniffo project.`;
		}
	}
