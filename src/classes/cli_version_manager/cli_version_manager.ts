...
export class classCliVersionManager {
	...
	public async autoSetDispatch() {
		...
		const details = this.getUniffoDetails(this.requiredCliVersion);
	...
	/**
	 * The `init` function initializes the Uniffo Version Manager by resetting values, creating a
	...
	public async setDefaultVersion(tagName: version) {
		...
		const uniffoVersionFilenameDepreciated = `${this.cliDir.versions}/${tagName}/Uniffo`;
		logger.debugVar('uniffoVersionFilenameDepreciated', uniffoVersionFilenameDepreciated);

		const uniffoVersionFilename = `${this.cliDir.versions}/${tagName}/uniffo`;
		logger.debugVar('uniffoVersionFilename', uniffoVersionFilename);

		if (await pathExist(uniffoVersionFilenameDepreciated)) {
			logger.debug('Rename', uniffoVersionFilenameDepreciated, uniffoVersionFilename);
			await Deno.rename(uniffoVersionFilenameDepreciated, uniffoVersionFilename);
		}

		const uniffoFilename = `${this.cliDir.main}/uniffo`;
		logger.debugVar('uniffoFilename', uniffoFilename);

		const uniffoTmpFilename = `${this.cliDir.main}/tmp_uniffo`;
		logger.debugVar('uniffoTmpFilename', uniffoTmpFilename);

		const uniffoToRmFilename = `${this.cliDir.main}/rm_uniffo`;
		logger.debugVar('uniffoToRmFilename', uniffoToRmFilename);
		...
		logger.debug('Copy', uniffoVersionFilename, uniffoTmpFilename);
		Deno.copyFileSync(uniffoVersionFilename, uniffoTmpFilename);

		if (await pathExist(uniffoFilename)) {
			logger.debug('Rename', uniffoFilename, uniffoToRmFilename);
			Deno.renameSync(uniffoFilename, uniffoToRmFilename);
		}

		logger.debug('Rename', uniffoTmpFilename, uniffoFilename);
